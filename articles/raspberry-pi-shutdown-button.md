...
GPIO21 (pin 40) ----[ Push Button ]---- GND

GPIO10 (pin 19) ----[ 330Ω ]----|>|---- GND
                                LED
```
...
最初は動く形を優先して、その後に整理するでもよいと思います。  
記事では「まずは動かす」「必要なら改善する」という流れにすると読みやすそうです。
...
- 使用した Raspberry Pi の機種
- Raspberry Pi OS のバージョン
- 実際の配線写真
- LED の抵抗値
- `shutdown-button.service` の実ファイル内容
- LED が消えてから何秒待って電源を切っているか

...
:::details 「WSL インストールが壊れている可能性があります」と表示されたら（WSLの修復について）
筆者がPCを初期化した直後にこの手順を試したところ、`wsl --install` を実行した瞬間に「WSL インストールが壊れている可能性があります」というエラーが出ました。
...
修復が完了したら、もう一度 `wsl --install` を実行すれば正常に進みます。PC初期化直後でなければ起きないと思いますが、同じ状況になった方は焦らず対処してください。
:::

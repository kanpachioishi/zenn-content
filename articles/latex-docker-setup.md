...
:::message
**PCを初期化した直後の場合**、`wsl --install` を実行すると「任意のキーを押すとWSLの修復を始めます」というメッセージが表示されることがあります。これはWSLの内部コンポーネントが壊れている状態です。**何もしないと60秒でタイムアウトしてしまう**ので、メッセージが出たらすぐに何かキーを押してください。修復が完了したら、もう一度 `wsl --install` を実行すれば正常に進みます。

実際の画面はこちらです。

![PC初期化直後にwsl --installを実行したときのエラー画面](/images/wsl-install-error.png)
*`wsl --install` 実行時に「WSL インストールが壊れている可能性があります」と表示された例*
:::

### 1-3. Ubuntuの初期設定

インストールが完了すると、そのままUbuntuが起動し、ユーザー名とパスワードの設定を求められます。好きなものを入力してください（このユーザー名・パスワードはLinux用で、Windowsのものとは別です）。

![Ubuntuの初期設定画面（ユーザー名・パスワード入力）](/images/wsl-ubuntu-setup.png)
*インストール完了後、そのままUbuntuが起動してユーザー名とパスワードの設定を求められる*

:::message
筆者の環境では再起動なしでそのまま使えましたが、PCの状態によっては再起動を求められる場合があります。その場合は再起動後にスタートメニューから「Ubuntu」を開いてください。
:::

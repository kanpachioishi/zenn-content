...
インストールが終わったら、**PCを再起動**してください。再起動しないとWSL2が有効になりません。

### 1-4. Ubuntuの初期設定

再起動後、Ubuntuのウィンドウが自動で開く、またはPowerShell上でそのまま、ユーザー名とパスワードの設定を求められます。好きなものを入力してください（このユーザー名・パスワードはLinux用で、Windowsのものとは別です）。
